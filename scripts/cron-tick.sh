...
#
# All three endpoints are idempotent (FOR UPDATE + status guards / SKIP LOCKED),
# so running this alongside any other scheduler (e.g. n8n) is safe — no double
# refund, no duplicate delivery.
#
# Usage:  cron-tick.sh [retries|reconcile|all]    (default: all)
...
case "$MODE" in
    ...
    all) run_retries; run_reconcile ;;
    *) echo "[cron-tick] unknown mode '$MODE' (use retries|reconcile|all)" >&2; exit 2 ;;
esac
