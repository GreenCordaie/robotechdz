...
export function InitiateReturnModal({ isOpen, onClose, order }: InitiateReturnModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (montant <= 0 || montant > totalAmount) {
            setError(`Le montant doit être entre 1 et ${totalAmount.toLocaleString("fr-DZ")} DA`);
            return;
        }
                ...
                <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-gray-700 mb-1.5">
                            Montant à rembourser * <span className="text-gray-400 font-normal">(max {totalAmount.toLocaleString("fr-DZ")} DA)</span>
                        </label>
