...
import { reconcilePendingBsvOrders } from "@/services/bsv-reconciler.service";
...
async function handler(req: NextRequest): Promise<NextResponse> {
    ...
    try {
        const result = await reconcilePendingBsvOrders({ limit: 100 });
        return NextResponse.json({ success: true, data: result });
    } catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        console.error("[bsv-reconcile] failed:", err);
