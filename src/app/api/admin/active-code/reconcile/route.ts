...
import { reconcilePendingActiveCodeOrders } from "@/services/active-code-reseller-reconciler.service";
...
async function handler(req: NextRequest): Promise<NextResponse> {
    ...
    try {
        const result = await reconcilePendingActiveCodeOrders({ limit: 100 });
        return NextResponse.json({ success: true, data: result });
    } catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        console.error("[active-code-reconcile] failed:", err);
