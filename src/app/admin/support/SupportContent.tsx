...
import { Search, Eye, CheckCircle2, Clock, MessageSquare, RefreshCw } from "lucide-react";
...
import SupportConversationView from "./components/SupportConversationView";
...
export default function SupportContent({ initialTickets = [] }: SupportContentProps) {
    ...
    const [viewMode, setViewMode] = useState<"TICKETS" | "CONVERSATIONS">("CONVERSATIONS");
    ...
    const loadTickets = useCallback(async (isInitial = false) => {
        ...
        if (isSearchFocused.current && !isInitial) return;
        ...
        try {
            ...
            if (Array.isArray(data)) {
                setTickets(prev => {
                    if (prev.length === data.length) {
                        const isMatch = prev.every((t, i) =>
                            t.id === data[i].id &&
                            t.status === data[i].status &&
                            t.message === data[i].message
                        );
            ...
            if (isInitial) toast.error("Échec du chargement des tickets");
        } finally {
    ...
    useEffect(() => {
        ...
        const poll = async () => {
            if (viewMode === "TICKETS" && !document.hidden) {
                await loadTickets(false);
            }
            timer = setTimeout(poll, 30000); // 30s poll for tickets
        };
        poll();
    ...
    return (
        <div className="flex flex-col gap-6 p-6 max-w-[1200px] mx-auto min-h-screen">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <Button
                        ...
                        variant="flat"
                        size="sm"
                        className="bg-white/5 text-slate-400 hover:text-white"
            ...
            {viewMode === "CONVERSATIONS" ? (
                ...
                <>
                    <div className="flex bg-[#1a1614] p-1.5 rounded-2xl border border-white/5 shadow-xl self-start">
                        <button
