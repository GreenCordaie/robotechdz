...
import {
    createG2BulkPricingRuleAction,
    updateG2BulkPricingRuleAction,
    deleteG2BulkPricingRuleAction,
    setG2BulkUsdRateAction,
    simulateG2BulkPricingAction,
} from "./actions";
...
interface G2BulkPricingClientProps {
    initialRules: RuleRow[];
    initialRate: number;
    tiers: TierOption[];
}

export default function G2BulkPricingClient({ initialRules, initialRate, tiers }: G2BulkPricingClientProps) {
    ...
    async function refreshRules() {
        const { getAllG2BulkPricingRulesAction } = await import("./actions");
        const res = await getAllG2BulkPricingRulesAction({} as never);
        if (res && (res as { success?: boolean }).success) {
            setRules((res as { data: RuleRow[] }).data);
        }
    }
    ...
    async function handleSaveRate() {
        ...
        startTransition(async () => {
            const res = await setG2BulkUsdRateAction({ rate: trimmed });
            if ((res as { success?: boolean }).success) {
                setRate(Number(trimmed));
                toast.success(`Taux mis à jour : ${trimmed} DZD/USD`);
            } else {
                toast.error((res as { error?: string }).error || "Échec mise à jour");
            }
        });
    }
    ...
    async function handleDelete(row: RuleRow) {
        ...
        const res = await deleteG2BulkPricingRuleAction({ id: row.id });
        if ((res as { success?: boolean }).success) {
            toast.success("Règle supprimée");
            await refreshRules();
        } else {
            toast.error((res as { error?: string }).error || "Suppression refusée");
        }
    }
    ...
    const [simPriceUsd, setSimPriceUsd] = useState<string>("5.00");
    const [simCategory, setSimCategory] = useState<string>("retail");
    const [simBrand, setSimBrand] = useState<string>("amazon");
    const [simSku, setSimSku] = useState<string>("g2b__amazon_jp_25");
    ...
    async function runSimulation() {
        ...
        const res = await simulateG2BulkPricingAction({
        ...
        if ((res as { success?: boolean }).success) {
            setSimResult((res as { data: SimResult }).data);
        } else {
            setSimError((res as { error?: string }).error || "Échec simulation");
        }
    }
        ...
        <div className="space-y-6">
            <header className="space-y-1">
                ...
                <p className="text-sm text-slate-400">
                    Règles de markup appliquées aux produits G2Bulk (LoadBrain) côté shop revendeur.
                </p>
            </header>
            ...
            <Card className="bg-[#161616] border border-white/5">
                <CardBody className="space-y-3">
                    ...
                    <p className="text-xs text-slate-500">
                        Source : manuel · cache service 1 h · partagé avec BSV (shop_settings)
                    </p>
                </CardBody>
            </Card>
...
function RuleModal({ isOpen, onClose, editing, onSaved }: RuleModalProps) {
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            const res = isEdit
                ? await updateG2BulkPricingRuleAction({ ...payload, id: editing!.id })
                : await createG2BulkPricingRuleAction(payload);
            if ((res as { success?: boolean }).success) {
            ...
            <ModalContent>
                ...
                <ModalBody className="space-y-3">
                    ...
                    <Input
                        ...
                        description={scopeType === "global" ? "Le scope global utilise toujours '*'" : "Ex: 'retail', 'amazon', 'g2b__amazon_jp_25'"}
                    />
