...
async function run() {
    try {
        ...
        console.log('--- FETCHING LAST 10 EMAILS (NO FILTER) ---');
        const graphUrl = "https://graph.microsoft.com/v1.0/me/messages?$orderby=receivedDateTime desc&$top=10";
        ...
        if (!data.value || data.value.length === 0) {
            console.log('Aucun email trouvé du tout.');
            return;
        }

        for (const msg of data.value) {
            console.log(`[${msg.receivedDateTime}] De: ${msg.from.emailAddress.address} - Sujet: ${msg.subject}`);
        }
